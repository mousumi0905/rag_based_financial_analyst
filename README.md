...
As a Quant Analyst, I lived with a recurring problem: RBI and SEBI publish new circulars almost every day, and any one of them could change our strategy overnight. That meant hours of manual reading — every single day — just to catch what had changed.

While reading *"Attention Is All You Need"*, it clicked: what if a company had an in-house AI that automatically reads new documents, answers *"what changed today?"*, and sends out daily briefings — instead of someone combing through PDFs by hand?

This project is the first step toward that idea: a RAG-based assistant that reads dense financial and regulatory reports (RBI, SEBI, HDFC) and lets anyone ask natural-language questions, with answers grounded in — and cited back to — the source documents, in seconds instead of hours.
